...
import {
  getMediaImageFetchUrl,
  getMediaImageUploadUrl
} from "../../../../../lib/backend";
import {
...
export async function POST(request) {
  ...
  const contentType = requestedContentType || file.type || "image/jpeg";
  const init = await requestBackendJson(getMediaImageUploadUrl(), {
    method: "POST",
  ...
  const fetchResult = await requestBackendJson(
    `${getMediaImageFetchUrl()}?path=${encodeURIComponent(key)}`,
    { method: "GET" }
  );
